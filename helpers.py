    ...
    with open(sciezka, "w") as fh:
        fh.write(textwrap.dedent(f"""\
            ...
            if sys.argv[1] == "info":
                ...
                sys.exit(0)
            obraz = sys.argv[-1]
            # Prawdziwe gw wybiera przeksztalcenie po rozszerzeniu i
            # odrzuca nieznane - atrapa musi robic tak samo, inaczej
            # przepuscilaby blad, ktory na sprzecie zatrzymuje naped.
            znane = (".img", ".ima", ".adf", ".st", ".msa", ".d64", ".scp")
            if not obraz.lower().endswith(znane):
                print("** FATAL ERROR:")
                print("%s: Unrecognised file suffix" % obraz)
                sys.exit(1)
